...
namespace PixelLab
{
    public class ImageProcessor
    {
        ...
        // Unified Pipeline: Handles Color Filtering + Quantization together!
        public void ApplySystemFilters(string system, double channel1Scale, double channel2Scale, double channel3Scale, double channel4Scale, int targetTotalColors = 0)
        {
            ...
            Parallel.For(0, totalPixels, pixelIndex =>
            {
                ...
                if (targetTotalColors >= 2)
                {
                    finalBlue = (byte)Math.Clamp(Math.Round((double)finalBlue / step) * step, 0, 255);
                    finalGreen = (byte)Math.Clamp(Math.Round((double)finalGreen / step) * step, 0, 255);
                    finalRed = (byte)Math.Clamp(Math.Round((double)finalRed / step) * step, 0, 255);
                }
